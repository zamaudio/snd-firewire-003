 ...
 * */
static const u8 digiscrt(const u8 idx, const int off) {
	...
	const u8 nib[15] = {0x8, 0x7, 0x9, 0x6, 0xa, 0x5, 0xb, 0x4, 0xc, 0x3, 0xd, 0x2, 0xe, 0x1, 0xf};

	/** circular list for upper nibble. */
	const u8 hir[15] = {0x0, 0x6, 0xf, 0x8, 0x7, 0x5, 0x3, 0x4, 0xc, 0xd, 0xe, 0x1, 0x2, 0xb, 0xa};
	/** start offset for upper nibble mapping */
	const u8 hio[16] = {0, 11, 12, 6, 7, 5, 1, 4, 3, 0x00, 14, 13, 8, 9, 10, 2};
...
void digi_encode(u8 * const data, const int nch) {
	int c;
	u8 carry = 0x00;
	u8 idx = 0;
	int off = 0;
...
void digi_decode(u8 * const data, const int nch) {
	int c;
	u8 carry = 0x00;
	u8 idx = 0;
	int off = 0;
...
static void hexdump(u8 *data, int nch, char *annotation) {
	int c;
...
int main(int argc, char **argv) {
	const int nch = 18;
	u8 *snd = calloc(BYTE_PER_SAMPLE * nch, sizeof(u8));

	printf("# of channels: %d\n", 18);

	memset(snd, 0, BYTE_PER_SAMPLE * nch * sizeof(u8));
